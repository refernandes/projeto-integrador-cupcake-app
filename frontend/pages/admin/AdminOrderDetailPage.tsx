...
const AdminOrderDetailPage: React.FC<AdminOrderDetailPageProps> = ({ order, setRoute, updateOrderStatus }) => {
    ...
    return (
        <div className="p-4 space-y-4">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm">
                <h3 className="font-bold text-lg text-title mb-2">Alterar Status do Pedido</h3>
                 <label htmlFor="orderStatus" className="block text-sm font-medium text-body-text mb-1">Status Atual</label>
                 <select 
                    id="orderStatus" 
                    value={currentStatus}
            ...
            <button onClick={() => setRoute({ name: 'adminManageOrders' })} className="w-full text-center text-primary font-semibold py-2">Voltar</button>
        </div>
    );
};
